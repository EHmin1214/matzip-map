...
def og_profile(nickname: str):
    ...
    if coords:
        ...
        map_html = '<svg viewBox="0 0 300 160" class="map">' + "".join(sp) + "</svg>"
    else:
        map_html = '<div class="map empty"><span>지도 미리보기</span></div>'
...
body{{min-height:100vh;display:flex;align-items:center;justify-content:center;background:#faf9f6;font-family:'Manrope',sans-serif;padding:24px}}
...
.map{{width:100%;aspect-ratio:300/160;border-radius:12px;background:#f0efec;margin-bottom:18px;overflow:hidden}}
.map.empty{{display:flex;align-items:center;justify-content:center;font-style:italic;font-size:12px;color:#a8a29e}}
...
.name{{font-family:'Noto Serif',Georgia,serif;font-size:17px;font-weight:700;color:#2f3430;margin:0;line-height:1.2}}
