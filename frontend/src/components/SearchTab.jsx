...
export default function SearchTab({ onPlaceAdded }) {
  ...
  const handleSearch = async () => {
    ...
    setSearching(true);
    try {
      const res = await axios.get(`${API_BASE}/search-place/`, {
        params: { name: query.trim() },
      });
      if (res.data) {
        setResult(res.data);
      } else {
        setError("가게를 찾을 수 없어요");
      }
    } catch (e) {
  ...
  return (
    <div style={{
      ...
      background: "#f8f8f8", zIndex: 20,
      display: "flex", flexDirection: "column",
    }}>
      {/* 헤더 */}
      <div style={{
        background: "white", padding: "16px 20px 12px",
        borderBottom: "1px solid #f0f0f0",
      }}>
        <h2 style={{ margin: "0 0 12px", fontSize: 18, fontWeight: 800, color: "#222" }}>
          맛집 검색
        </h2>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
        ...
        {error && (
          <div style={{
            background: "#fff0ed", borderRadius: 10, padding: "12px 16px",
            fontSize: 14, color: "#E8593C", textAlign: "center",
          }}>
            {error}
          </div>
        )}
        ...
        {result && (
          <div style={{
            ...
            <div style={{ marginBottom: 16 }}>
              <p style={{ margin: "0 0 4px", fontSize: 12, color: "#E8593C", fontWeight: 600 }}>
                {result.category || "맛집"}
              </p>
              <h3 style={{ margin: "0 0 6px", fontSize: 20, fontWeight: 800, color: "#1a1a1a" }}>
                {result.name}
              </h3>
        ...
        {!result && !error && !savedMsg && (
          <div style={{ textAlign: "center", padding: "60px 20px", color: "#ccc" }}>
            ...
            <p style={{ fontSize: 14 }}>가게 이름으로 검색해보세요</p>
            <p style={{ fontSize: 12, marginTop: 4 }}>예: 을지면옥, 스타벅스 강남점</p>
          </div>
        )}
      </div>
